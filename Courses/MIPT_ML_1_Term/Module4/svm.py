...
def rbf(x_1, x_2, sigma=1.0):
    ...
    """
    x1 = torch.sum(x_1**2, dim=1, keepdim=True)
    x2 = torch.sum(x_2**2, dim=1, keepdim=True)
    distances = torch.exp(-x1 / 2 / sigma) @ torch.exp(-x2 / 2 / sigma).T
    # distances = torch.exp(-distances / 2 / sigma)  ### YOUR CODE HERE
...
class SVM(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, Y):
        ...
        for epoch in range(self.epochs):
            ...
            sum_loss = 0.0  # Loss for each epoch
            ...
            if self.verbose:
                print(
                    "Epoch " + str(epoch) + ", Loss: " + str(sum_loss / self.batch_size)
                )
